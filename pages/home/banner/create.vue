  ...
  <div class="page-container">
          ...
          <v-card outlined>
            <v-card-text>
              <v-btn-toggle v-model="select_language" mandatory dense color="primary">
                <v-btn v-for="lang in language_options" :key="lang.title" :value="lang">{{ lang.language }}</v-btn>
              </v-btn-toggle>
              <v-row>
                <v-col cols="12">
    ...
    <v-row class="mt-5">
      <v-col cols="12" class="text-right">
        <v-btn :disabled="isDataLoading" class="action" @click="goBack">
          <v-icon left>mdi-arrow-left-bold</v-icon>
          Back</v-btn
        >
        <v-btn class="action ml-2" color="primary" :disabled="isDataLoading" @click="submit(false)">
          <v-icon left>mdi-content-save</v-icon>
          {{ $t('action_save') }}</v-btn
        >
        <v-btn class="action ml-2" color="primary" :disabled="isDataLoading" @click="submit(true)">
          <v-icon left>mdi-content-save</v-icon>
          Save & Back</v-btn
        >
      </v-col>
...
<script>
import Create from '@/js/pages/home/banner/create.js';
